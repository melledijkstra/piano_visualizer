...
func _load_midi_data():
    ...
    var notes = MidiPreprocessor.preprocess(parser)
    song = Song.new(notes)
    print("Debug: Parsed ", song.notes.size(), " notes.")

    show_song_info()

    _generate_layout(song.notes)

func show_song_info():
    if song_info == null or song.notes.is_empty():
        return
    song_info.text = \
    "Notes: " + str(song.notes.size()) + \
    "\nTotal song time: " + song.duration_formatted()
...
func _generate_layout(notes: Array[NoteData]):
    ...
    timeline.custom_minimum_size.x = max_x * 1.05 # Add padding
...
func _on_zoom_changed(value: float):
    ...
    if timeline:
        timeline.custom_minimum_size.x = max_x * 1.05
