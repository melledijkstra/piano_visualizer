	...
	if debug_output != "0":
		...
		while true:
			...
			if status == MIDI_PARSER_EOB:
				...
				var midi_header = instance.header
				debug += "header\n"
				debug += "    size: "+str(midi_header.size)+"\n"
				debug += "    format: "+str(midi_header.format)+"\n"
				debug += "    tracks count: "+str(midi_header.tracks)+"\n"
				debug += "    time division: "+str(midi_header.time_division)+"\n"
				continue
...
class Event:
	...
	var event_type: EventType = EventType.UNKNOWN
	var additional_data = {}
...
class Meta extends Event:
	...
	var ms_per_tick: int = 60000 / (120 * 480) # set in case of a SET_TEMPO event
